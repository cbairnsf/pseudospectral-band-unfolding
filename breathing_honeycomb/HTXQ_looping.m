...
%	Q = k_H (H - E I)^2 + k_T (T - e^(ika) I)^2 + k_X (X - x I)^2
...
%				operators
%	Example Usage:
%		data_out = HTXQ_looping(H, T, X, 1, 1, 0.5, Evals, kvals, xvals); 
function data_out = HTXQ_looping(H, T, X, k_H, k_T, k_X, H_pev, T_pev, X_pev)
	...
	total_num_iterations = lX * lT * lX;					%The total number of quadratic gaps 
	...
	%	end
		...
		for jndex = 1:lT						%Middle loop (k direction)
			...
			for kndex = 1:lX					%Inner loop (x direction)
				...
				%eval_Q = realsqrt(real(eigs(Q, 1, 'sa')));	%Compute the quadratic gap
				...
				data_out(index, jndex, kndex, :) = [eval_Q;...	%Store the data
							...
							varT;
							varX];
				%data_out(index, jndex, kndex) = eval_Q;		%Add data to data_out using linear index
				if prog_bar 
